...
function single_find(word){
    ...
    oDowner.fetch(word, 1).then(function([[word, o], timeout]){
        oDowner.fetchCigen(word).then(function(o, [[word, cigen], timeout]){
            var endTime = Date.parse(new Date())
            var diff = (endTime - starTime) / 1000
            atom.clipboard.write(oDowner.to_final_string(o, cigen))
...
function batch_find(wordList){
    ...
    oDowner.batchFind(wordList, function(res){
        oDowner.batchFindCiGen(wordList, function(res, res2){
            var l = res.flat()
            var myMap = new Map(l)
            var result = []
            for (let [w, cigen] of res2.flat()) {
                var o = myMap.get(w)
                if(o){
                    result.push(oDowner.to_final_string(o, cigen))
                }
            }
            // %%
            var txt = result.join('\n')
...
export function get_single_cigen(){
    ...
    if (editor) {
        ...
        if (selectedText) {
            ...
            oDowner.get_single_cigen(selectedText).then(function([[word, cigen], timeout]){
                var endTime = Date.parse(new Date())
                var diff = (endTime - starTime) / 1000
                // var data = r['cigen']
                console.log(cigen);
                atom.clipboard.write(cigen.substr(3).replace('\n', ' '))
...
export function get_single_phonetic(){
    ...
    if (editor) {
        ...
        if (selectedText) {
            ...
            var starTime = Date.parse(new Date())
            var oDowner = new Dict(selectedText)
            oDowner.get_single_phonetic(selectedText).then(function(r){
                var endTime = Date.parse(new Date())
                var diff = (endTime - starTime) / 1000
                var data = `${selectedText} ${r['phonetic']}`
                editor.selectWordsContainingCursors()
                editor.insertText(data)
                atom.clipboard.write(`${selectedText} ${r['phonetic']}  ${r['answer']}`)
