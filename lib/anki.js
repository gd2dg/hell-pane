...
export function parseAnki(firstLine, content, matchArray, idx){
    ...
    if (list.length > 0) {
        ...
        if(idx2 > 0){
            sPhonetic = `<span id="phonetic"> ${list[1]} </span> [sound:${sWord.substr(0, idx2).trim()}.mp3]`
        }
        else{
            sPhonetic = `<span id="phonetic"> ${list[1]} </span> [sound:${sWord}.mp3]`
        }
    }
    var tagName = Common.getFileName()
    var entryList = preDealAnswer(content)
    console.log(entryList);
    var answerList = []
    var quotient = parseInt(entryList.length / weights.len)
    if(quotient == 0) quotient = 1
    var tmpList = []
    for (var i = 0; i < weights.len; i++) {
