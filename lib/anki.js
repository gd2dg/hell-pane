...
function parseContent(vList){
    ...
    var matchArray
    var aList = []
    if(fistLine && (matchArray = fistLine.match(/(.*) -- (.*)/)) != null){
        // 首行确定 类型 英文解释 中文解释
        ...
        var sFirst =  `${sTypeHtml} ${sChiTransHtml} ${sEnTransHtml} `
        // 其余行作为例子
        for (var i = 1; i < vList.length; i++) {
            aList.push(`${vList[i]}`)
        }
        var sExampleHtml = `<p class='example'> ${aList.join('<br>')} </p>`
        return `${sFirst} ${sExampleHtml}`
    }
    else{
