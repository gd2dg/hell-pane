...
export function parseAnki(firstLine, content, matchArray, idx){
    ...
    if((mArray = title.match(headReg)) != null){
        sOringin = mArray[1].trim()
        if (sOringin.split(' ').length > 1){
            var isTerm = true
        }
        else{
            isTerm = false
        ...
        var sNote = mArray[2]
        var sIgnoreOrigin = mArray[3]
        if(sIgnoreOrigin && sNote){
            sWord = `${sNote.substring(1, sNote.length-1)}`
        }
        else if (sNote && sNote != '<note>') {
            sWord = `${sOringin} ${sNote.substring(1, sNote.length-1)}`
        }
        else{
            sWord = sOringin
        }
        if(mArray[4] ){
            if (!isTerm) {
                sPhonetic = `<span id="phonetic"> ${mArray[4]} [sound:${sOringin}.mp3] </span> `
            }
            else
                sPhonetic = `<span id="phonetic"> ${mArray[4]}</span> `
        }
    }
    ...
    if (isTerm) {
        var result =  `${sWord}\t${''}\t${sPhonetic}\t${sAddtionalPa} ${answer}\t${tagName}\n`
    }
    else
        result =  `${sWord}\t${sOringin}\t${sPhonetic}\t${sAddtionalPa} ${answer}\t${tagName}\n`
...
function parseExample(lines){
    ...
    var hExample = '', hOExample = '', hOther=''
    if (example) {
        hExample = `<p class='example'>${example}</p>`
        hOExample = `<p class='example-other'>${lines.splice(i+1).join('<br>')}</p>`
        hOther = `<p class='other'>${otherList.join('<br>')}</p>`
        return `${hExample}${hOExample}${hOther}`
    }
    else{
        hOther = `<p class='other'>${lines.join('<br>')}</p>`
        return hOther
    }
}
...
function gen_word_block_html(o, indx){
    ...
    if(o.paraList && o.paraList.length > 0 ){
        var sExampleHtml = parseExample(o.paraList)
        // if(o.type){
    ...
    if(o.enTrans){
        if(!o.paraList || o.paraList.length == 0 ){
            var sEnTransHtml = `<p class='example-dir'>${o.enTrans}</p>`
        }
        else{
