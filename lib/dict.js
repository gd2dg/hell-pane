...
export default class dict extends Downer {
    ...
    init(word, i) {
        ...
        this.haiUrl = `https://dict.cn/${querystring.escape(word)}`
    }
    ...
    get_brief(word){
        ...
        return this.download(this.dictUrl, this.do_get_brief_with_youdao.bind(this))
    }

    do_get_brief_with_youdao(resolve, docRoot){
    ...
    get_single_cigen(word){
        return this.download(this.dictUrl, this.do_get_single_cigen_with_youdao.bind(this))
        // return this.download(this.cigenUrl, this.do_download_cigen.bind(this))
    }

    do_get_single_cigen_with_youdao(resolve, docRoot){
        var oResult = this.getBasicResultObject(docRoot)
        // var basicTrans = oResult['basicTrans'].split('\n').join('  ')
        // var sFinal = `${oResult['title']} ${oResult['phonetic']} ${basicTrans}`
        // var sFinal = `${sTitle} ${sPhonetic}`
    ...
    do_download_dict(resolve, docRoot){
        ...
        var notifyStr = `${this.idx}: ${oResult['title']} is done!!, already num ${global.already}, time_out_num:${global.tmout}`
    ...
    getBasicResultObject(docRoot){
        ...
        try {
            ...
            if (sTitle) {
                // if it is found in collins, add the answer
                oResult['answer'] = this.get_answer(docRoot)
            }
            else{
                // if not use the basic trans
