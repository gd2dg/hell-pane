...
export default class downer {
    ...
    to_doms(body, decode){
        if (decode) {
            body = iconv.decode(body, decode)
        }
        else{
            body = iconv.decode(body)
        }
    ...
    // common_do(resolve, docRoot, owner, call_back){
    //     var sFinal = ''
    //     try {
    //         sFinal = call_back(docRoot)
    //     } catch (e) {
    //         sFinal = 'not found'
    //     } finally {
    //         resolve(sFinal)
    ...
    do_paser_page_url(resolve, body, owner){
        ...
        var sFinal = []
        try {
            sFinal = owner.do_paser_page_url2(docRoot, owner)
        } catch (e) {
            console.log(e);
        } finally {
            resolve(sFinal)
    ...
    do_get_brief(resolve, body, owner){
        ...
        try {
            ...
            sAnswer = owner.get_brief_desc(docRoot)
        } catch (e) {
            sFinal = ''
        } finally {
            sFinal = `${sTitle} ${sPhonetic} ${sAnswer}`
            resolve(sFinal)
        }
    }
    ...
    do_download_dict(resolve, body, owner){
        ...
        try {
            ...
            oPromise.then((sCiGen) =>{
                ...
                o['cigen'] = sCiGen
                if (o['phonetic'] != '') {
                    o['link'] = owner.to_phonetic_link(o['title'])
                }
                else{
                    o['link'] = ''
                }
                var sFinal = owner.to_final_string(o)
                var notifyStr = `${owner.idx}: ${o['title']} is done!!`
