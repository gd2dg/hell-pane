...
export default class downer {
    ...
    parse_collins_entry(li){
        ...
        var sExampleHtml = `<p class="example">${finalExample}<p>`
    ...
    parseTrans(oTrans){
        ...
        var regex = /([^\u4e00-\u9fa5]*)([\u4e00-\u9fa5].*)/
        var matchArray = sTrans.match(regex)
        if(matchArray!=null){
            console.log(matchArray);
            var sTypeHtml = `<span class="w-type">${typeString}</span>`
            var sEnTrans = matchArray[1]
            var sChiTrans = matchArray[2]
            var sEnTransHtml = `<span class="en-trans">${sEnTrans}</span>`
            var sChiTransHtml = `<span class="chi-trans">${sChiTrans}</span>`
            return `${sTypeHtml} -- ${sEnTransHtml} ${sChiTransHtml} `
        }
        else{
